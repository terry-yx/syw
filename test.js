...
window.scencename = '开心神兽';
window.geturl = [
    'http://39.108.28.79/cgi_bin',
    'http://39.108.28.79/cgi_bin'
    //'http://192.168.0.126:7001',
    //'http://192.168.0.126:7001'
]
// window.http_url =  'http://dev.supreme-treasure.com/cgi_bin';
window.appId = 'wxf4ac37e669ad4c77';
window.XlappId = '';
window._ishjlm = false;//皇家联盟
window.openChat = true;//打开聊天室
window.newPlatform = true;  //新平台
window._canSavePlayType = false;  //保存玩法
window.ws_url = 'ws://dev.supreme-treasure.com/web_sk2';
window.CardToPoint = 1;
;
...
var ws_url = 'ws://47.107.27.98/web_sk';
...
function onImgClick(event) {
    ...
    // alert(isHengPhoto);
    if(isHengPhoto){
        //  x = e.screenX / window.screen.height;
        // y = e.screenY / window.screen.width;
        // // console.log('x--->' + x + '----y----->' + y);
        // // return;
        // console.log('x轴点击位置--------'+  e.screenX);
        // console.log(window.screen.height * 0.7)
        // console.log(window.screen.height * 0.3)
        // if( e.screenX > window.screen.height * 0.7){
                // window.ScoreBoard.toHall(); 
        // }else if(e.screenX > window.screen.height * 0.1 && e.screenX < window.screen.height * 0.3){
...
function initWindowFunctions() {
    ...
    function getDomain(platform){
        http.post({ url: window.http_url +'/authDomain', data: {'platform':platform}, timeout: 5000 }, function (err, result) {
        if (err) {
            alert('授权失败！请刷新界面重新登录！');
        } else {
            ...
            var urlhref = 'http://' + result.data +'/cgi_bin?platform='+platform+'&url='+sendUrl;
            console.log(urlhref);
...
///<jscompress sourcefile="wechat-1.2.0.js" />
...
///<jscompress sourcefile="jquery.swipebox.js" />
